...
//-> Web / Mobile
exports.tambahNilai = async (traceId, id_sekolah, { nisn, nama, kelas, mapel, semester, tugas, uh, uts, uas }) => {
  let message = "";
  try {
    // validasi body
    if (!(nisn && nama && kelas && mapel && semester && tugas && uh && uts && uas)) {
      message = "body (nisn, nama, kelas, mapel, semester, tugas, uh, uts, uas) require!";
    ...
    // render
    message = "berhasil menambah nilai siswa!";
    print(traceId, { message });
    return {
      code: 200,
      message,
...
// Update
exports.updateNilai = async (traceId, { id, mapel, semester, tugas, uh, uts, uas }) => {
  let message = "";
  try {
    // validasi body
    if (!(id && mapel && semester && tugas && uh && uts && uas)) {
      message = "body (id, mapel, semester, tugas, uh, uts, uas) require!";
      print(traceId, { message });
      return {
        code: 400,
        message,
